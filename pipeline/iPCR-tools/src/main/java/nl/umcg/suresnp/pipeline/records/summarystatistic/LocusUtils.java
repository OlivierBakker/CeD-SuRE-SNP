...
import nl.umcg.suresnp.pipeline.records.summarystatistic.filters.PvalueFilterSmaller;
import nl.umcg.suresnp.pipeline.records.summarystatistic.filters.RegionFilter;
import nl.umcg.suresnp.pipeline.records.summarystatistic.filters.SummaryStatisticsRecordFilter;
import nl.umcg.suresnp.pipeline.records.summarystatistic.filters.VariantIdFilter;
...
public class LocusUtils {
    ...
    public static List<Locus> selectIndepTopHits(String[] args, RandomAccessGenotypeData reference, Map<String, SummaryStatisticRecord> records) throws LdCalculatorException {
        ...
        for (Locus topLocus : topLoci) {
            ...
            GeneticVariant iVar = reference.getSnpVariantByPos(indep.getContig().replace("chr", ""), (int) indep.getPosition());
            GeneticVariant oVar;
            ...
            for (SummaryStatisticRecord record : topLocus.getRecords().values()) {
                ...
                for (String indepVar : topLocus.getIndepVariants()) {
                    ...
                    iVar = reference.getSnpVariantByPos(indep.getContig().replace("chr", ""), (int) indep.getPosition());
                    oVar = reference.getSnpVariantByPos(record.getContig().replace("chr", ""), (int) record.getPosition());
    ...
    public static Map<String, Double> selectTaggingVariants(RandomAccessGenotypeData genotypeData, SummaryStatisticRecord record, int window, double ldThreshold) throws LdCalculatorException {
        ...
        Iterable<GeneticVariant> locus = genotypeData.getVariantsByRange(record.getContig().replace("chr", ""), (int) (record.getPosition() - window), (int) (record.getPosition() + window));

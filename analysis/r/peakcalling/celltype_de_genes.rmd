...
count.matrix           <- fread("data/SuRE-SNP_GW_x3_filtered.countMatrix", data.table = F)
...
```{r}
celltype.peaks        <- read.table(file="output/celltype_specific_sure_peaks_genome_wide_lfc_gt_1.tsv", sep="\t", header=T, stringsAsFactors =F)
celltype.peaks         <- celltype.peaks[celltype.peaks$baseMean > 500,]
...
write.table(closest.gene, file="output/celltype_de_peaks_genome_wide_closest_tss_5kb_window.tsv", sep="\t", quote=F)
```

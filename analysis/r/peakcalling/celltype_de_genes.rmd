...
celltype.peaks          <- results.table[results.table$specificity < 0.8,]
rownames(celltype.peaks) <- celltype.peaks$peak
```
...
celltypes.of.interest <- c(grep("epi", unique(marker.genes$cell.type), value=T), grep("T ", unique(marker.genes$cell.type), value=T), "Erythroid-like and erythroid precursor cells")
...
marker.sites$specificity <- celltype.peaks[marker.sites$peak, "specificity"]
```

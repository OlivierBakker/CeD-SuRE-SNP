...
k562         <- read.ase.results("output/summary_statistics/k562_ase_wilcox_overlapped_with_peaks_x3.assoc")
caco         <- read.ase.results("output/summary_statistics/caco_ase_wilcox_overlapped_with_peaks_x3.assoc")
caco.stim    <- read.ase.results("output/summary_statistics/cacoStim_ase_wilcox_overlapped_with_peaks_x3.assoc")

raw.ase.data <- fread("zcat output/SuRE_CeD_SNP.minimal.allele.specific.ipcr.gz", header=F, sep="\t") 
snps         <- as.character(fread("../variant_calling/output/unique_snps.txt", data.table=F, sep="\t", header=F)[,1])
```
...
pdf(width=11, height=3.5, file="output/plots/sure_snp_rs140496_ase.pdf")

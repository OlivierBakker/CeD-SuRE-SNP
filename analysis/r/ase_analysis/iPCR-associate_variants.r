  ...
  if (length(table(cur.x$V6)) == 2) {
    if (min(table(cur.x$V6)) > 12) {
        ...
        if (sum(cooks.dist > cutoff) > 0.25*nrow(x)) {
           out <- c(snp, unlist(cur.x[1,3:5]), unlist(names(table(cur.x$V6))), table(cur.x$V6), 0,0,0,1,0,0,0)
        } else {
           out <- c(snp, unlist(cur.x[1,3:5]), unlist(names(table(cur.x$V6))), table(cur.x$V6), summary(nb.reg)$coefficients[3,], summary(nb.reg)$coefficients[1,1], nb.reg$theta, nb.reg$`SE.theta`)
        } 
    } else {
      out <- c(snp, unlist(cur.x[1,3:5]), unlist(names(table(cur.x$V6))), table(cur.x$V6), 0,0,0,1,0,0,0)
    }
  } else {
    out <- c(snp, unlist(cur.x[1,3:5]), "NA", "NA", "NA", "NA", 0,0,0,1,0,0,0)
  }

  names(out) <- c("SNP","CHR", "BP", "BP_IN_READ", "A1", "A2", "COUNT_A1", "COUNT_A2", "BETA", "SE", "Z", "P", "INTERCEPT", "THETA", "SE_THETA")
  return(out)
...
df.out           <- data.frame(matrix(unlist(out), ncol=15, byrow = T), stringsAsFactors=F)
rownames(df.out) <- df.out$SNP
colnames(df.out) <- c("SNP","CHR", "BP", "BP_IN_READ", "A1", "A2", "COUNT_A1", "COUNT_A2", "BETA", "SE", "Z", "P", "INTERCEPT", "THETA", "SE_THETA")
df.out           <- df.out[,c("SNP","CHR", "BP", "P", "BETA", "SE", "Z", "A1", "A2", "COUNT_A1", "COUNT_A2", "INTERCEPT", "THETA", "SE_THETA")]
df.out$BP        <- as.numeric(df.out$BP)
df.out           <- df.out[!is.na(df.out$BP),]
df.out           <- df.out[order(df.out$CHR, df.out$BP),]

write.table(df.out, sep="\t", file="SuRE_CeD_SNP_K562_negbinom_covar_outlier_rm.assoc", quote=F, row.names=F)
